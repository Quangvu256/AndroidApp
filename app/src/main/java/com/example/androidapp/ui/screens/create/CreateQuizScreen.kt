...
fun CreateQuizScreen(
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            // Both options are only meaningful for published quizzes. While the quiz is in
            // draft mode (isDraft == true) the toggles are shown but disabled, and a hint
            // explains that they take effect only on publish.
            item {
                PublishOptionsSection(
                    isPublic = uiState.isPublic,
                    shareToPool = uiState.shareToPool,
                    isDraft = uiState.isDraft,
                    onIsPublicChanged = { viewModel.onEvent(CreateQuizEvent.IsPublicChanged(it)) },
                    onShareToPoolChanged = { viewModel.onEvent(CreateQuizEvent.ShareToPoolChanged(it)) }
                )
            }
